...
describe('UNKNOWN Namespace - SQZMOM Method', () => {
    it('should calculate SQZMOM correctly with native series and variable series', async () => {
        ...
        const { result, plots } = await pineTS.run((context) => {
            ...
            const _v1 = ta.highest(high, lengthKC);
            const _v2 = ta.lowest(low, lengthKC);
            const _v3 = ta.sma(close, lengthKC);
            const _v4 = math.avg(math.avg(_v1, _v2), _v3);
            const _v5 = source - _v4;
            //const val = ta.linreg(_v5, lengthKC, 0);
